...
export default async function AdminTopicsPage() {
  ...
  return (
    <main className="container-shell py-10">
      ...
      <h1 className="mt-2 text-4xl font-semibold tracking-normal text-[var(--ink)]">Topics</h1>
      <AdminNav />
      <div className="grid gap-4">
        {topics.map((topic) => (
          <div key={topic.id} className="rounded-md border border-[var(--line)] bg-[var(--bg-elevated)] p-5">
            <div className="flex flex-wrap justify-between gap-3">
              <h2 className="text-xl font-semibold text-[var(--ink)]">{topic.title}</h2>
              <span className="rounded-full bg-[var(--bg)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--ink-muted)]">
                {topic.status}
              </span>
            </div>
            <p className="mt-2 leading-7 text-[var(--ink-soft)]">{topic.summary}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {topic.keywords.map((keyword) => (
                <span key={keyword} className="rounded-full bg-[var(--bg)] px-3 py-1 text-xs text-[var(--ink-soft)]">
                  {keyword}
                </span>
              ))}
            </div>
