...
export function ReviewQueue({ tasks }: { tasks: ReviewTask[] }) {
  if (!tasks.length) {
    return (
      <div className="rounded-md border border-line bg-white p-5 text-sm text-ink/65">
  ...
  return (
    <div className="grid gap-3">
      {tasks.map((task) => (
        <div key={task.id} className="rounded-md border border-line bg-white p-5">
          ...
          <p className="mt-2 text-sm text-ink/65">{task.reason}</p>
