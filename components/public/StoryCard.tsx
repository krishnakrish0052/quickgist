...
export function StoryCard({ article, size = "medium", priority = false }: StoryCardProps) {
    ...
    return (
      <article className="story-card group relative overflow-hidden rounded-2xl bg-ink">
        ...
        <div className="absolute inset-x-0 bottom-0 p-8 md:p-12">
          ...
          <Link href={href}>
            <h2 className="mt-4 max-w-3xl font-display text-[clamp(1.75rem,4vw,3rem)] font-bold leading-[1.08] tracking-tight text-white transition group-hover:text-white/90">
              {article.title}
            </h2>
          </Link>
          {article.dek ? (
    ...
    return (
      <article className="group flex flex-1 items-center gap-3 py-3">
        <div className="min-w-0 flex-1">
          <CategoryPill category={article.category} />
          <Link href={href}>
            <h3 className="mt-1 font-display text-[0.85rem] font-semibold leading-snug text-ink transition group-hover:text-signal line-clamp-2">
              {article.title}
            </h3>
          </Link>
          <div className="mt-1">
        ...
        <Link href={href} className="relative h-[60px] w-[84px] shrink-0 overflow-hidden rounded-lg bg-line">
    ...
    return (
      <article className="story-card group grid overflow-hidden rounded-2xl border border-line/60 bg-white md:grid-cols-[1.4fr_1fr]">
        <Link href={href} className="relative block aspect-[16/9] w-full overflow-hidden bg-line md:aspect-auto md:min-h-[240px]">
          <Image src={heroImg} alt="" fill sizes="(max-width:768px) 100vw, 55vw" className="object-cover transition duration-500 group-hover:scale-[1.03]" />
        </Link>
  ...
  return (
    <article className="story-card group flex h-full flex-col overflow-hidden rounded-2xl border border-line/60 bg-white">
      <Link href={href} className={`relative block ${aspectClass} w-full shrink-0 overflow-hidden bg-line`}>
        <Image
          src={heroImg}
          alt=""
          fill
